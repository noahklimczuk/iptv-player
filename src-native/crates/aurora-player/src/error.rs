...
/// A playback failure translated for humans (README §17): what happened, the likely
...
mod tests {
    ...
    fn distinguishes_auth_from_connection_limit() {
        assert_eq!(
            PlaybackError::classify("HTTP 401").code,
            ErrorCode::Unauthorized
        );
        assert_eq!(
            PlaybackError::classify("provider reports max connections reached").code,
            ErrorCode::ConnectionLimit
        );
    }
    ...
    fn drm_is_never_retryable_and_says_so_plainly() {
        let e = PlaybackError::classify("stream is encrypted (widevine)");
    ...
    fn bad_credentials_offer_settings_not_retry() {
        let e = PlaybackError::classify("401 Unauthorized");
        assert!(!e.retryable);
    ...
    fn dead_streams_can_be_reported() {
        let e = PlaybackError::classify("HTTP 404 not found");
    ...
    fn transient_failures_are_retryable() {
        for raw in [
            ...
            assert!(PlaybackError::classify(raw).retryable, "{raw} should retry");
        }
    }
    ...
    fn every_error_has_a_message_a_cause_and_an_action() {
        for raw in [
            ...
            let e = PlaybackError::classify(raw);
