...
Aurora TV is free software under the **GNU General Public License, version 3 or
later** (`LICENSE`). It ships with, and links against, software under other licences.
This file is the notice those licences require. It is bundled with the installer and
...
The full text of the LGPL-2.1 is in `licenses/LGPL-2.1.txt`; the GPL-3.0 is in
`licenses/GPL-3.0.txt` and at the repository root as `LICENSE`.

### The relinking obligation, and how this build satisfies it
...
be statically linked** into the executable without re-reading the LGPL and satisfying
section 6 some other way.

### Source

mpv's source, including the precise revision each Windows build was made from, is at
<https://github.com/mpv-player/mpv>. The Windows builds Aurora's release workflow
fetches are published at <https://github.com/shinchiro/mpv-winbuild-cmake>, which also
publishes the build scripts and the exact component versions for each archive.

FFmpeg's source is at <https://ffmpeg.org/download.html> and
<https://github.com/FFmpeg/FFmpeg>.

If you received a binary of Aurora and want the corresponding source for any of the
above and cannot obtain it from those addresses, open an issue on Aurora's repository
and it will be provided.

---

## The Mozilla Public License components

Four Rust crates in the dependency tree are under the **MPL-2.0**, which requires that
the source of *those files* be made available if they are modified. Aurora does not
modify them; they are used as published, and their source is at the repositories
listed against them below.

| Crate | Purpose |
|---|---|
| `cssparser`, `cssparser-macros`, `selectors` | CSS parsing inside the web layer |
| `dtoa-short` | number formatting, used by the above |
| `option-ext` | used by the platform directory lookup |

---

## Metadata and artwork

Film and series metadata, posters and backdrops come from **TMDB**
(<https://www.themoviedb.org>). *This product uses the TMDB API but is not endorsed or
certified by TMDB.* TMDB's terms of use apply to that data, and Aurora caches it
locally only as an accelerator.

Guide data comes from whichever XMLTV source the viewer's provider publishes. Aurora
neither supplies nor endorses any provider, playlist or stream.

---

## Rust dependencies

The workspace links **504 crates**. The overwhelming majority are dual-licensed MIT
or Apache-2.0. The full list, with the licence each one declares, follows.

<details>
<summary>All 504 crates</summary>
...
Only the runtime dependencies are listed: the build tooling is not shipped.
