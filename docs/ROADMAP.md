...
| 7 — Series | Seasons, episodes, detail tabs | **Done.** Next-episode autoplay and skip-intro are not implemented. |
...
| Playback state machine and error taxonomy | 18 `aurora-player` tests |
| The mpv/Win32 backend compiles for Windows | `cargo check --target x86_64-pc-windows-msvc` |
| Every screen renders and the journeys work | 12 Playwright runs against the production bundle |
| The Tauri host compiles | **Not verified here** — needs the `windows` CI job |
...
3. **Profiles**, which most of §11 depends on.
